...
#   >>> review {workdir}/{iso}/prep/{iso}.contam_report.tsv + *.blob.png/*hist.png , then create
#       two curated files in {iso}/prep/ (empty = keep all; FIX/REVIEW go in NEITHER -> kept whole):
#         {iso}.remove.txt : confident whole-contig EXCLUDEs, one id per line  (dropped whole)
#         {iso}.trim.rpt   : TRIM-only FCS rows = report header + TRIM lines    (ends trimmed)
